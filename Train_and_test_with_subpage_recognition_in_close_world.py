...
from sklearn.metrics import precision_score, recall_score, f1_score  # Remove accuracy_score
...
def hierarchical_loss(main_preds, sub_preds, main_labels, sub_labels):
    ...
    Scenarios 1: 2-tab Chrome datasets: main_loss: 2.5, sub_loss: 1.0, constraint_loss: 0.2
    Scenarios 2: 3-tab Chrome datasets: main_loss: 1.5, sub_loss: 1.0, constraint_loss: 0.2
    Scenarios 3: 2-tab Tor datasets: main_loss: 0.5, sub_loss: 1.0, constraint_loss: 0.2
    """
...
def main():
    ...
    def train_one_epoch(epoch):
        ...
        main_map_at_k = calculate_mapk(main_targets, main_probs_fused, k= )  # Set k as per scenario (see Table 1 in the paper)
        ...
        sub_map_at_k = calculate_mapk(sub_targets, sub_probs_fused, k= )  # Set k as per scenario (see Table 1 in the paper)
    ...
    # 4. Evaluation function (only modify prediction structure, others unchanged)
    def evaluate(loader, set_name="Test set"):
        ...
        main_p_at_k = calculate_pak(main_targets, main_probs_fused, k= )          # Set k as per scenario (see Table 1 in the paper)
        main_map_at_k = calculate_mapk(main_targets, main_probs_fused, k= )       # ISet k as per scenario (see Table 1 in the paper)
        ...
        sub_p_at_k = calculate_pak(sub_targets, sub_probs_fused, k= )                 # Set k as per scenario (see Table 1 in the paper)
        sub_map_at_k = calculate_mapk(sub_targets, sub_probs_fused, k= )              # Set k as per scenario (see Table 1 in the paper)
    ...
    Scenarios 1: 2-tab Chrome datasets: 100 epochs
    Scenarios 2: 3-tab Chrome datasets: 200 epochs
    Scenarios 3: 2-tab Tor datasets: 150 epochs
    """
